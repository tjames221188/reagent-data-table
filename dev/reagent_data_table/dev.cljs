...
(def child-row-opts
  {:child-row-render-fn (fn [person]
                          (when (not-empty (:info person))
                            [:td {:col-span "3"}
                             [:table.subtable
                              [:tbody
                               (for [[k v] (:info person)]
                                 ^{:key (str person "-info-" (name k))}
                                 [:tr [:td (name k)] [:td v]])]]]))})
...
(defn- toggle-child-rows
  ...
  (assoc-in app [:table-data :child-rows]
            (if (-> app :table-data :child-rows)
              (do (println "Removing Child rows") nil)
              (do (println "Adding child rows") child-row-opts))))
  ...
  [:div
   ...
   [:div.table-container
    [rdt/data-table {:headers    [[:id "ID"] [:name "Name"] [:age "Age"]]
                     ...
                     :child-rows (-> @app :table-data :child-rows)
                     :table-id   "foo-table"}]]
